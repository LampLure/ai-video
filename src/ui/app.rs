...
impl AiVideoApp {
    ...
    fn poll_model_health(&mut self) { let Some(check) = self.pending_model_check.clone() else { return; }; if Instant::now() < check.due { return; } self.pending_model_check = None; let ready = models::is_llama_service_ready(Duration::from_millis(300)); match (check.kind, ready) { (ModelCheckKind::Start, true) => { self.model_status = "7080 服务已连接，模型运行正常。".to_string(); self.show_notice("模型启动成功", "已成功连接 127.0.0.1:7080。".to_string()); } (ModelCheckKind::Start, false) => { self.model_status = "未能连接 7080，请检查脚本和模型日志。".to_string(); self.show_notice("模型可能未就绪", "启动命令已发送，但当前无法连接 127.0.0.1:7080。模型可能仍在加载，或脚本启动失败。".to_string()); } (ModelCheckKind::Stop, false) => { self.model_status = "7080 已释放。".to_string(); self.show_notice("7080 已释放", "检测到 127.0.0.1:7080 已不可连接。".to_string()); } (ModelCheckKind::Stop, true) => { self.model_status = "7080 仍可连接，可再次点击终止。".to_string(); self.show_notice("7080 仍可连接", "强制终止后 7080 仍可连接。可以再次点击“终止”。".to_string()); } } }
...
impl Drop for AiVideoApp { fn drop(&mut self) { if let Some(cancel) = &self.ai_cancel { cancel.store(true, Ordering::Relaxed); } if let Some(mut child) = self.model_child.take() { models::stop_model_process(&mut child); } } }
